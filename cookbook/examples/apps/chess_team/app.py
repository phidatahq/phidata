...
def display_move_history():
    ...
    if "move_history" in st.session_state and st.session_state.move_history:
        ...
        moves_text = []
        current_move = {"number": 1, "white": "", "white_piece": "", "black": "", "black_piece": ""}
        
        for move in st.session_state.move_history:
            if move["player"] == "White":
                if current_move["white"]:
                    moves_text.append(current_move)
                    current_move = {"number": len(moves_text) + 1, "white": "", "white_piece": "", "black": "", "black_piece": ""}
                current_move["white"] = move["move"]
                piece_name = move.get("piece", "")
                if piece_name in piece_symbols:
                    current_move["white_piece"] = piece_symbols[piece_name][0]  # White piece symbol
            else:
                current_move["black"] = move["move"]
                piece_name = move.get("piece", "")
                if piece_name in piece_symbols:
                    current_move["black_piece"] = piece_symbols[piece_name][1]  # Black piece symbol
                moves_text.append(current_move)
                current_move = {"number": len(moves_text) + 1, "white": "", "white_piece": "", "black": "", "black_piece": ""}
        ...
        for move in moves_text:
            white = f"{move['white_piece']} {move['white']}" if move['white_piece'] else move['white']
            black = f"{move['black_piece']} {move['black']}" if move['black_piece'] else move['black']
...
def display_game_status():
    ...
    if "game_started" in st.session_state and st.session_state.game_started:
        ...
        agents = {
            "White Piece Agent": "Waiting for next move" if len(st.session_state.move_history) % 2 == 0 else "Thinking...",
            "Black Piece Agent": "Thinking..." if len(st.session_state.move_history) % 2 == 1 else "Waiting for next move",
        ...
        # Show current turn
        current_turn = "White" if len(st.session_state.move_history) % 2 == 0 else "Black"
        st.sidebar.markdown(f"**Current Turn:** {current_turn}")

def check_game_ending_conditions(board_state: str, legal_moves: str, current_color: str) -> bool:
...
Respond with appropriate status."""
            ...
            response_content = master_response.content.strip() if master_response else ""
...
Choose the best move from these legal moves. Respond ONLY with your chosen move."""
                ...
                if response_content:
                    ...
                    if ai_move not in legal_moves_list:
                        logger.error(f"Chosen move {ai_move} not in legal moves! Available moves: {legal_moves_list}")
                        st.warning(f"Invalid move choice by {agent_name}, retrying... (Attempt {retry_count + 1}/{max_retries})")
                    ...
                    if "successful" in result.get("status", ""):
                        ...
                        add_move_to_history(ai_move, "White" if is_white_turn else "Black", result)
                        ...
                        st.warning(f"Invalid move by {agent_name}, retrying... (Attempt {retry_count + 1}/{max_retries})")
                        return play_next_move(retry_count + 1, max_retries)
                else:
                    logger.error("No response content from agent")
                    st.warning(f"No response from {agent_name}, retrying... (Attempt {retry_count + 1}/{max_retries})")
                    return play_next_move(retry_count + 1, max_retries)
                    
        except Exception as e:
            logger.error(f"Error during agent move: {str(e)}")
            st.warning(f"Error during {agent_name}'s move, retrying... (Attempt {retry_count + 1}/{max_retries})")
            return play_next_move(retry_count + 1, max_retries)
