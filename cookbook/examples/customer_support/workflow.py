...
class CustomerSupportWorkflow(Workflow):
    ...
    def return_order(self, order_id: int, user_email: str) -> str:
        response = self.database_agent.run(f"Return order {order_id} for user {user_email} if the order status is COMPLETED else cancel the order")
        return response.content
    
    @Workflow.register(description="Cancel an order")
    def cancel_order(self, order_id: int, user_email: str) -> str:
        response = self.database_agent.run(f"Cancel order {order_id} for user {user_email} if the order status is ORDERED or DISPATCHED")
        return response.content
