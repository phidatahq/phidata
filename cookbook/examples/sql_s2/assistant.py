...
# USERNAME = "admin"
# PASSWORD = "Single!123"
# HOST = "svc-ff3a3e21-c36b-4ad4-9f62-bd10c4e07669-dml.aws-virginia-6.svc.singlestore.com"
# PORT = 3306
# DATABASE = "demo_db"
...
# ************* Storage & Knowledge *************
assistant_storage = S2AssistantStorage(
    schema="demo_db",
...
def get_sql_assistant(
    ...
    return Assistant(
        ...
        # llm=OpenAIChat(base_url='http://localhost:1234/v1', temperature=0),
        # llm=OllamaTools(model="llama3-gradient"),
        # llm=Hermes(model="openhermes"),
        ...
        description=dedent(
            """\
        You are an expert SQL Engineer called `SQrL` and your goal is to help users analyze data using SingleStoreSQL queries.
        ...
        instructions=[
            ...
            "If you need to run a query, **THINK STEP BY STEP** about how to accomplish the task using the `ai` database ",
            ...
            "Using the table information and rules, create one single syntactically correct SingleStoreSQL query to accomplish your task.",
            "Remember: ALWAYS FOLLOW THE TABLE RULES. NEVER IGNORE THEM. IT IS CRITICAL THAT YOU FOLLOW THE `table rules` if provided.",
            "If you need to join tables, check the `ai` database for the relationships between the tables."
            + "\n  - If the `ai` database contains a relationship between tables, use that relationship to join the tables even if the column names are different."
            + "\n  - If you cannot find a relationship in the `ai` database, use `describe_table` and only join on the columns that have the same name and data type."
            ...
            "If the users asks about the tables you have access to, simply share the table names from the `ai` database.",
        ],
