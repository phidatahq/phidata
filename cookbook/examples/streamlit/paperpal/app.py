    ...
    if "topic" in st.session_state:
        ...
        with st.status("Generating Search Terms", expanded=True) as status:
            with st.container():
                ...
                search_terms = search_term_generator.run(json.dumps(search_generator_input))  # type: ignore
                if search_terms:
            ...
            with st.status("Searching Exa", expanded=True) as status:
                with st.container():
                    ...
                    try:
                        ...
                        if exa_search_results and len(exa_search_results.content.results) > 0:  # type: ignore
                            exa_content = exa_search_results.model_dump_json(indent=4)
                            exa_container.json(exa_search_results.content.results)  # type: ignore
        ...
        if search_arxiv:
            with st.status("Searching ArXiv (this takes a while)", expanded=True) as status:
                with st.container():
                    ...
                    if arxiv_search_results and arxiv_search_results.content.results:  # type: ignore
                        arxiv_container.json([result.model_dump() for result in arxiv_search_results.content.results])  # type: ignore
                status.update(label="ArXiv Search Complete", state="complete", expanded=False)

            if arxiv_search_results and arxiv_search_results.content.results:  # type: ignore
                paper_summaries = []
                for result in arxiv_search_results.content.results:  # type: ignore
                    summary = {
                        "ID": result.id,
                        "Title": result.title,
