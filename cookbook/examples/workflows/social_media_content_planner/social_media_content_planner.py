...
# Define agents
# Blog Analyzer Agent:
# Extracts blog content (title, sections) and converts it into Markdown format for further use.
blog_analyzer = Agent(
...
# Creates a Twitter thread from the blog content, each tweet is concise, engaging, and logically connected with relevant media.
twitter_thread_planner = Agent(
    ...
    verbose=agents_config['twitter_thread_planner']['verbose'],
)

# LinkedIn Post Planner:
# Converts blog content into a structured LinkedIn post, optimized for a professional audience with relevant hashtags.
linkedin_post_planner = Agent(
    ...
    verbose=agents_config['linkedin_post_planner']['verbose'],
)
...
class ContentPlanningWorkflow(Workflow):
    ...
    blog_post_url: str = Field(..., description="URL of the blog post to analyze.")
    post_type: str = Field(..., description="Type of post to generate (e.g., twitter or linkedin).")
    ...
    def generate_plan(self, blog_content: str, post_type: str) -> dict:
        if post_type == "twitter":
            ...
            plan_response: RunResponse = twitter_thread_planner.run(blog_content)
        elif post_type == "linkedin":
            print(f"Generating post plan for {post_type}")
            plan_response: RunResponse = linkedin_post_planner.run(blog_content)
        else:
            raise ValueError(f"Unsupported post type: {post_type}")

        if plan_response.content_type == "Thread":
            return plan_response.content

        if plan_response.content_type == "application/json":
