"""Run `pip install ollama exa_py duckduckgo-search yfinance pypdf sqlalchemy 'fastapi[standard]' phidata youtube-transcript-api` to install dependencies."""
...
local_agent_storage_file: str = "local_agents.db"
...
research_agent = Agent(
    ...
    - [Reference 2](link)
    """),
    storage=SqlAgentStorage(table_name="research_agent", db_file=local_agent_storage_file),
    add_history_to_messages=True,
    add_datetime_to_instructions=True,
    markdown=True,
)
...
app = Playground(agents=[web_agent, finance_agent, research_agent, youtube_agent]).get_app()

if __name__ == "__main__":
    serve_playground_app("ollama:app", reload=True)
