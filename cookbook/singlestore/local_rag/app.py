...
from assistant import get_local_rag_assistant  # Assuming assistant.py is in the same directory
from logging import getLogger

logger = getLogger(__name__)

st.set_page_config(
    page_title="Local RAG with SingleStore",
    page_icon=":orange_heart:",
)
st.title("Local RAG with SingleStore")
st.markdown("##### :orange_heart: Built using [phidata](https://github.com/phidatahq/phidata) and SingleStore")
...
def main() -> None:
    ...
    try:
        ...
        logger.error(f"Could not create assistant run: {e}")
    ...
    if local_rag_assistant.storage:
        ...
        if st.session_state["local_rag_assistant_run_id"] != new_local_rag_assistant_run_id:
            logger.info(f"---*--- Loading run: {new_local_rag_assistant_run_id} ---*---")
