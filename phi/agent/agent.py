...
from phi.model import LanguageModel, LanguageModelResponse
...
class Agent(BaseModel):
    ...
    # -*- Model settings
    # LanguageModel to use for this Agent
    lm: Optional[LanguageModel] = Field(None, alias="language_model")
    ...
    # add_history_to_messages=true adds the chat history to the messages sent to the LanguageModel.
    ...
    # Tools are functions the LanguageModel may generate JSON inputs for.
    # If you provide a dict, it is not called by the LanguageModel.
    tools: Optional[List[Union[Tool, Toolkit, Callable, Dict, Function]]] = None
    # Show tool calls in Agent response.
    show_tool_calls: bool = False
    # Maximum number of tool calls allowed.
    tool_call_limit: Optional[int] = None
    # Controls which (if any) tool is called by the LanguageModel.
    # "none" means the LanguageModel will not call a tool and instead generates a message.
    # "auto" means the LanguageModel can pick between generating a message or calling a tool.
    ...
    # -*- Default tools
    # Add a tool that allows the LanguageModel to read the chat history.
    read_chat_history: bool = False
    # Add a tool that allows the LanguageModel to search the knowledge base (aka Agentic RAG)
    search_knowledge: bool = False
    # Add a tool that allows the LanguageModel to update the knowledge base.
    update_knowledge: bool = False
    # Add a tool that allows the LanguageModel to get the tool call history.
    read_tool_call_history: bool = False

    # -*- Extra Messages
    # A list of extra messages added after the system prompt and before the user prompt.
    # Use these for few-shot learning or to provide additional context to the LanguageModel.
    ...
    def update_lm(self) -> None:
        if self.lm is None:
            try:
                ...
                logger.exception(e)
                logger.error(
                    "phidata uses `openai` as the default LanguageModel. "
        ...
        # Set response_format if it is not set on the LanguageModel
        if self.output_model is not None and self.lm.response_format is None:
            self.lm.response_format = {"type": "json_object"}

        # Add tools to the LanguageModel
        agent_tools = self.get_tools()
        if agent_tools is not None:
            for tool in agent_tools:
                self.lm.add_tool(tool)

        # Set show_tool_calls if it is not set on the LanguageModel
        if self.lm.show_tool_calls is None and self.show_tool_calls is not None:
            self.lm.show_tool_calls = self.show_tool_calls

        # Set tool_choice to auto if it is not set on the LanguageModel
        if self.lm.tool_choice is None and self.tool_choice is not None:
            self.lm.tool_choice = self.tool_choice

        # Set tool_call_limit if set on the agent
        if self.tool_call_limit is not None:
            self.lm.tool_call_limit = self.tool_call_limit

        # Add session_id to the LanguageModel
        if self.session_id is not None:
            self.lm.session_id = self.session_id
    ...
    def get_models_dict(self) -> Dict[str, Any]:
        return {
            "lm": self.lm.to_dict() if self.lm is not None else None,
        }
    ...
    def from_agent_session(self, session: AgentSession):
        ...
        # Update models from the AgentSession
        if session.models is not None:
            # Update model metrics from the AgentSession
            if "lm" in session.models:
                lm_metrics_from_db = session.models.get("metrics")
                if lm_metrics_from_db is not None and isinstance(lm_metrics_from_db, dict) and self.lm:
                    try:
                        self.lm.metrics = lm_metrics_from_db
                    except Exception as e:
                        logger.warning(f"Failed to load models from AgentSession: {e}")
    ...
    def get_system_message(self) -> Optional[Message]:
        ...
        5. Build the default system message for LanguageModel.
        """
        ...
        if self.lm is None:
            raise Exception("LanguageModel not set")
        ...
        # 4.5 Add instructions specifically from the LanguageModel
        model_instructions = self.lm.get_instructions_from_model()
        ...
        # 5.3 Then add the prompt specifically from the LanguageModel
        system_prompt_from_model = self.lm.get_system_prompt_from_model()
    ...
    def _run(
        ...
        **kwargs: Any,
        ...
        2. Update the LanguageModel (set defaults, add tools, etc.)
        3. Prepare the List of messages to send to the LanguageModel
        ...
        4. Generate a response from the LanguageModel (includes running function calls)
        5. Update Memory
        ...
        # 2. Update the LanguageModel (set defaults, add tools, etc.)
        self.update_lm()
        run_response.models = {"lm": self.lm.model if self.lm is not None else None}

        # 3. Prepare the List of messages to send to the LanguageModel
        messages_for_lm: List[Message] = []

        # 3.1. Add the System Message to the messages list
        system_message = self.get_system_message()
        if system_message is not None:
            messages_for_lm.append(system_message)

        # 3.2 Add extra messages to the messages list
        if self.add_messages is not None:
            for _m in self.add_messages:
                if isinstance(_m, Message):
                    messages_for_lm.append(_m)
                elif isinstance(_m, dict):
                    try:
                        messages_for_lm.append(Message.model_validate(_m))
                    except Exception as e:
                        logger.warning(f"Failed to validate message: {e}")

        # 3.3 Add chat history to the messages list
        if self.add_history_to_messages and self.memory is not None:
            messages_for_lm += self.memory.get_last_n_run_messages(last_n=self.num_history_messages)
        ...
        if message is not None:
            ...
            if isinstance(message, Message):
                messages_for_lm.append(message)
            # If message is provided as a str, build the user message
                ...
                if user_message is not None:
                    messages_for_lm.append(user_message)
        ...
        num_messages_to_skip = len(messages_for_lm) - 1

        # 4. Generate a response from the LanguageModel (includes running function calls)
        lm_response: LanguageModelResponse
        self.lm = cast(LanguageModel, self.lm)
        if stream and self.streamable:
            lm_response = LanguageModelResponse(content="")
            for model_response_chunk in self.lm.response_stream(messages=messages_for_lm):
                if model_response_chunk.content is not None and lm_response.content is not None:
                    lm_response.content += model_response_chunk.content
                    run_response.content = model_response_chunk.content
                    run_response.messages = messages_for_lm
                    yield run_response
        else:
            lm_response = self.lm.response(messages=messages_for_lm)
            run_response.content = lm_response.content
            run_response.messages = messages_for_lm

        # Add metrics to the run_response
        run_response.metrics = self.lm.metrics if self.lm else None
        # 5. Update Memory
        ...
        assistant_message = Message(role="assistant", content=lm_response.content)
        # Add Assistant Message to the chat history
        self.memory.add_chat_message(message=assistant_message)

        # Add messages from this particular run to the memory
        run_messages = messages_for_lm[num_messages_to_skip:]
        ...
        if stream:
            self.run_response.content = lm_response.content
        ...
        functions = {}
        if self.lm is not None and self.lm.functions is not None:
            for _f_name, _func in self.lm.functions.items():
        ...
        if self.monitoring:
            run_data = {
                ...
                "functions": functions,
                "metrics": self.lm.metrics if self.lm else None,
            }
        else:
            run_data = {
                "functions": functions,
    ...
    async def _arun(
        ...
        **kwargs: Any,
        ...
        if stream and self.streamable:
            lm_response = LanguageModelResponse(content="")
            model_response_stream = self.lm.aresponse_stream(messages=messages_for_lm)
            ...
            lm_response = await self.lm.aresponse(messages=messages_for_lm)
            run_response.content = lm_response.content
            run_response.messages = messages_for_lm
    ...
    def generate_session_name(self) -> str:
        ...
        generated_name: LanguageModelResponse = self.lm.response(messages=generate_name_messages)
