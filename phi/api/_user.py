...
async def authenticate_and_get_user(
    ...
    async with aiohttp.ClientSession(base_url=phi_cli_settings.api_url, headers=authenticated_headers) as session:
        async with session.post("/v1/user/cliauth") as response:
...
def sign_in_user(
    sign_in_data: EmailPasswordAuthSchema,
) -> Optional[UserSchema]:
    from phi.cli.credentials import save_auth_token
