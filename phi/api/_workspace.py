...
from phi.api.httpx_client import base_headers, get_authenticated_client
...
def get_primary_workspace(user: UserSchema) -> Optional[WorkspaceSchema]:
    ...
    with authenticated_client as api:
        try:
            r: Response = api.post(ApiRoutes.WORKSPACE_READ_PRIMARY, data=user.model_dump(include={"id_user", "email"}))
            if is_invalid_response(r):
                return None
        except NetworkError:
            log_network_error_msg()
...
def get_available_workspaces(user: UserSchema) -> Optional[List[WorkspaceSchema]]:
    ...
    with authenticated_client as api:
        try:
            logger.debug(f"Sending dict: {user.model_dump(include={'id_user', 'email'})}")
            logger.debug(f"Sending json: {user.model_dump_json(include={'id_user', 'email'})}")
            r: Response = api.post(
                ApiRoutes.WORKSPACES_READ_AVAILABLE, data=user.model_dump_json(include={"id_user", "email"})
            )
            if is_invalid_response(r):
                return None
        except NetworkError:
        ...
        ws_list_dict = r.json()
        logger.debug(f"ws_list_dict: {ws_list_dict}")
        ...
        if is_valid_response(r):
            ...
            for ws_dict in ws_list_dict:
                if not isinstance(ws_dict, dict):
                    ...
                    continue
                ws_list.append(WorkspaceSchema.model_validate(ws_dict))
            return ws_list
