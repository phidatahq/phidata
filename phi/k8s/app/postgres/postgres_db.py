...
class PostgresDb(DbApp):
    ...
    def get_db_password(self) -> Optional[str]:
        ...
        if db_password_var is None and self.args.secrets_file is not None:
            ...
            secret_data_from_file = self.get_secret_data()
            if secret_data_from_file is not None:
                db_password_var = secret_data_from_file.get("POSTGRES_PASSWORD", db_password_var)
    ...
    def get_docker_rg(self, docker_build_context: Any) -> Optional[Any]:
        ...
        if docker_build_context is None or not isinstance(docker_build_context, DockerBuildContext):
        ...
        if self.args.postgres_host_auth_method:
            container_env["POSTGRES_HOST_AUTH_METHOD"] = self.args.postgres_host_auth_method
        ...
        if self.args.postgres_initdb_args_file:
            container_env["POSTGRES_INITDB_ARGS_FILE"] = self.args.postgres_initdb_args_file
        ...
        if self.args.open_container_port:
            container_ports[str(self.args.container_port)] = self.args.container_host_port
    ...
    def get_k8s_rg(self, k8s_build_context: Any) -> Optional[Any]:
        ...
        if k8s_build_context is None or not isinstance(k8s_build_context, K8sBuildContext):
        ...
        storage_classes: List[CreateStorageClass] = self.args.extra_storage_classes or []
        ...
        sa_name: Optional[str] = self.args.sa_name or k8s_build_context.service_account_name
        ...
        if self.args.open_container_port:
            container_port = CreatePort(
                ...
                target_port=self.args.service_target_port or self.args.container_port_name,
            )
            ...
            # If ServiceType == LOAD_BALANCER then validate self.args.service_node_port only IF available
                ...
                if self.args.service_node_port is not None:
                    if self.args.service_node_port < 30000 or self.args.service_node_port > 32767:
                        logger.error(f"NodePort: {self.args.service_node_port} invalid")
                        logger.error(f"Skipping this service")
                        return None
                    else:
        ...
        if self.args.container_labels is not None and isinstance(self.args.container_labels, dict):
        ...
        pg_container = CreateContainer(
            ...
            args=[self.args.command] if isinstance(self.args.command, str) else self.args.command,
            # Equivalent to docker images ENTRYPOINT
            command=[self.args.entrypoint] if isinstance(self.args.entrypoint, str) else self.args.entrypoint,
            image_pull_policy=self.args.image_pull_policy or ImagePullPolicy.IF_NOT_PRESENT,
        ...
        if self.args.pod_annotations is not None and isinstance(self.args.pod_annotations, dict):
            pod_annotations.update(self.args.pod_annotations)

        deploy_labels: Dict[str, Any] = common_labels or {}
        if self.args.deploy_labels is not None and isinstance(self.args.deploy_labels, dict):
        ...
        recreate_deployment_on_update = (
            ...
            if (
                ...
                and self.args.volume_type in (PostgresVolumeType.AwsEbs, PostgresVolumeType.AWS_EBS)
            )
            else False
        )
        ...
        if self.args.create_service:
            ...
            if self.args.service_labels is not None and isinstance(self.args.service_labels, dict):
