...
class Groq(Model):
    ...
    def invoke(self, messages: List[Message]) -> Any:
        ...
        formatted_tools = [m.to_dict() for m in messages]
        for i in formatted_tools:
            i.pop("created_at")
        return self.client.chat.completions.create(
            model=self.model,
            messages=formatted_tools,  # type: ignore
            **self.api_kwargs,
        )
    ...
    def response(self, messages: List[Message]) -> ModelResponse:
        ...
        if agent_message.tool_calls is not None and len(agent_message.tool_calls) > 0:
