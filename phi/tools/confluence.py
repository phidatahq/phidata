...
class ConfluenceTools(Toolkit):
    def __init__(
        ...
        if not self.server_url:
            logger.error(
                "Confluence server URL not provided. Please set the CONFLUENCE_SERVER_URL environment variable."
            )
        ...
        self.confluence = Confluence(url=self.server_url, username=self.username, password=self.password, cloud=True)
