...
class FileTools(Toolkit):
    def __init__(
        ...
        if write_files:
            self.register(self.write_to_file, sanitize_arguments=False)
        if read_files:
            self.register(self.read_file)
        if list_files:
            self.register(self.list_files)

    def write_to_file(self, contents: str, file_name: str, append: bool = True) -> str:
        """Saves the contents to a file called `file_name` and returns the file name if successful.

        :param contents: The contents to save.
        :param file_name: The name of the file to save to.
        :param append: Append to the file if it exists, otherwise overwrite the file. Defaults to True.
        ...
        try:
            ...
            if file_path.exists() and append:
                file_path.write_text(file_path.read_text() + contents)
            else:
