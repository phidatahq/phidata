...
class AlertManagerArgs(PhidataAppArgs):
    name: str = "alertmanager"
    version: str = "1"
    enabled: bool = True

    # -*- Image Configuration
    image_name: str = "prom/alertmanager"
    image_tag: str = "v0.25.0"
...
class AlertManager(PhidataApp):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        try:
            self.args: AlertManagerArgs = AlertManagerArgs(
                name=name,
                version=version,
                enabled=enabled,
                image=image,
                ...
                if workspace_volume_name is None:
                    if workspace_name is not None:
                        workspace_volume_name = get_default_volume_name(
                            f"alertmanager-{workspace_name}-ws"
                        )
                    else:
                        workspace_volume_name = get_default_volume_name(
                            "alertmanager-ws"
    ...
    def get_k8s_rg(self, k8s_build_context: Any) -> Optional[Any]:
        ...
        if self.args.mount_workspace:
            ...
            if workspace_volume_name is None:
                if workspace_name is not None:
                    ...
                    workspace_volume_name = get_default_volume_name("alertmanager-ws")
        ...
        containers.insert(0, alertmanager_container)

        # Set default container for kubectl commands
        # https://kubernetes.io/docs/reference/labels-annotations-taints/#kubectl-kubernetes-io-default-container
        pod_annotations = {
            "kubectl.kubernetes.io/default-container": alertmanager_container.container_name,
        }
        ...
        alertmanager_deployment = CreateDeployment(
        ...
        deployments.append(alertmanager_deployment)
