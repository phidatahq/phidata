...
class AmundsenFrontendArgs(PhidataAppArgs):
    name: str = "amundsenfrontend"
    version: str = "1"
    enabled: bool = True

    # -*- Image Configuration
    image_name: str = "amundsendev/amundsen-frontend"
    image_tag: str = "4.2.0"
    entrypoint: Optional[Union[str, List]] = None
    command: Optional[Union[str, List]] = None

    # -*- AmundsenFrontend Configuration

class AmundsenFrontend(PhidataApp):
    def __init__(
        self,
        name: str = "amundsenfrontend",
        version: str = "1",
        ...
        **kwargs,
        ...
        super().__init__()
        try:
            self.args: AmundsenFrontendArgs = AmundsenFrontendArgs(
                name=name,
                version=version,
                enabled=enabled,
                image=image,
                ...
                if workspace_volume_name is None:
                    if workspace_name is not None:
                        workspace_volume_name = get_default_volume_name(
                            f"AmundsenFrontend-{workspace_name}-ws"
                        )
                    else:
                        workspace_volume_name = get_default_volume_name("AmundsenFrontend-ws")
    ...
    def get_k8s_rg(self, k8s_build_context: Any) -> Optional[Any]:
        ...
        # Create the AmundsenFrontend container
        AmundsenFrontend_container = CreateContainer(
        ...
        containers.insert(0, AmundsenFrontend_container)

        # Set default container for kubectl commands
        # https://kubernetes.io/docs/reference/labels-annotations-taints/#kubectl-kubernetes-io-default-container
        pod_annotations = {
            "kubectl.kubernetes.io/default-container": AmundsenFrontend_container.container_name,
        }
        ...
        AmundsenFrontend_deployment = CreateDeployment(
        ...
        deployments.append(AmundsenFrontend_deployment)
        ...
        if self.args.create_service:
            ...
            _service = CreateService(
                ...
                deployment=AmundsenFrontend_deployment,
                ports=ports if len(ports) > 0 else None,
