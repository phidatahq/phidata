...
class AmundsenMetadataArgs(PhidataAppArgs):
    name: str = "amundsenmetadata"
    version: str = "1"
    enabled: bool = True

    # -*- Image Configuration
    image_name: str = "amundsendev/amundsen-metadata"
    image_tag: str = "4.0.2"
    entrypoint: Optional[Union[str, List]] = None
    command: Optional[Union[str, List]] = None

    # -*- AmundsenMetadata Configuration

class AmundsenMetadata(PhidataApp):
    def __init__(
        self,
        name: str = "amundsenmetadata",
        version: str = "1",
        ...
        **kwargs,
        ...
        super().__init__()
        try:
            self.args: AmundsenMetadataArgs = AmundsenMetadataArgs(
                name=name,
                version=version,
                enabled=enabled,
                image=image,
                ...
                if workspace_volume_name is None:
                    if workspace_name is not None:
                        workspace_volume_name = get_default_volume_name(
                            f"AmundsenMetadata-{workspace_name}-ws"
                        )
                    else:
                        workspace_volume_name = get_default_volume_name("AmundsenMetadata-ws")
    ...
    def get_k8s_rg(self, k8s_build_context: Any) -> Optional[Any]:
        ...
        # Create the AmundsenMetadata container
        AmundsenMetadata_container = CreateContainer(
        ...
        containers.insert(0, AmundsenMetadata_container)

        # Set default container for kubectl commands
        # https://kubernetes.io/docs/reference/labels-annotations-taints/#kubectl-kubernetes-io-default-container
        pod_annotations = {
            "kubectl.kubernetes.io/default-container": AmundsenMetadata_container.container_name,
        }
        ...
        AmundsenMetadata_deployment = CreateDeployment(
        ...
        deployments.append(AmundsenMetadata_deployment)
        ...
        if self.args.create_service:
            ...
            _service = CreateService(
                ...
                deployment=AmundsenMetadata_deployment,
                ports=ports if len(ports) > 0 else None,
