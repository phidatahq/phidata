...
class RedisArgs(DbAppArgs):
    ...
    volume_type: RedisVolumeType = RedisVolumeType.EmptyDir
    ...
    # EbsVolume if volume_type = RedisVolumeType.AWS_EBS
...
class Redis(DbApp):
    ...
    def get_docker_rg(self, docker_build_context: Any) -> Optional[Any]:
        ...
        if self.args.create_volume:
            if self.args.volume_type == RedisVolumeType.EmptyDir:
            ...
            elif self.args.volume_type == RedisVolumeType.HostPath:
                if self.args.volume_host_path is not None:
                    ...
                    logger.error("Redis: volume_host_path not provided")
                    return None
            else:
    ...
    def get_k8s_rg(self, k8s_build_context: Any) -> Optional[Any]:
        ...
        if self.args.create_volume:
            ...
            if self.args.volume_type == RedisVolumeType.EmptyDir:
                ...
                RedisVolumeType.AwsEbs,
                RedisVolumeType.AWS_EBS,
            ):
                if (
                    ...
                    logger.error("Redis: ebs_volume not provided")
                    return None
            else:
                logger.error(f"{self.args.volume_type.value} not supported")
                return None
        ...
        # Add Redis container to the front of the containers list
