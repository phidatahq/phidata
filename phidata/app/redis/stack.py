...
class RedisStackVolumeType(ExtendedEnum):
    HostPath = "HostPath"
    EmptyDir = "EmptyDir"
    AwsEbs = "AwsEbs"


class RedisStackArgs(DbAppArgs):
    ...
    # EbsVolume if volume_type = RedisStackVolumeType.AWS_EBS
...
class RedisStack(DbApp):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        try:
            self.args: RedisStackArgs = RedisStackArgs(
                name=name,
                version=version,
                enabled=enabled,
    ...
    def get_docker_rg(self, docker_build_context: Any) -> Optional[Any]:
        ...
        # Create a volume for the redis data
        if self.args.create_volume:
            if self.args.volume_type == RedisStackVolumeType.EmptyDir:
            ...
            elif self.args.volume_type == RedisStackVolumeType.HostPath:
    ...
    def get_k8s_rg(self, k8s_build_context: Any) -> Optional[Any]:
        ...
        if self.args.create_volume:
            ...
            if self.args.volume_type == RedisStackVolumeType.EmptyDir:
                ...
                if (
                    ...
                    logger.error("RedisStack: ebs_volume not provided")
                    return None
            else:
                logger.error(f"{self.args.volume_type.value} not supported")
                return None
        ...
        # Create the RedisStack container
        redis_container = CreateContainer(
        ...
        # Add RedisStack container to the front of the containers list
        containers.insert(0, redis_container)

        # Set default container for kubectl commands
        # https://kubernetes.io/docs/reference/labels-annotations-taints/#kubectl-kubernetes-io-default-container
        pod_annotations = {
            "kubectl.kubernetes.io/default-container": redis_container.container_name,
        }
        ...
        # Create the RedisStack deployment
