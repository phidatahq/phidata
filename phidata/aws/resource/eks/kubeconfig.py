...
from phidata.k8s.resource.kubeconfig import (
    Kubeconfig,
    KubeconfigCluster,
...
class EksKubeconfig(AwsResource):
    ...
    def get_resource_name(self) -> Optional[str]:
        return self.name if self.name else f"{self.eks_cluster.name}-kubeconfig"
    ...
    #     if aws_client.aws_region is not None:
    #         new_user_exec_args.extend(["--region", aws_client.aws_region])
    #     # Assume the role using the role_arn if provided
    #     if self.role_arn is not None:
    #         new_user_exec_args.extend(["--role-arn", self.role_arn])
    #     # Otherwise if role is provided, use that to get the role arn
    #     elif self.role is not None:
    #         _arn = self.role.get_arn(aws_client=aws_client)
    #         if _arn is not None:
    #             new_user_exec_args.extend(["--role-arn", _arn])
    #
    #     new_user_exec: Dict[str, Any] = {
    ...
    #     cluster_config: KubeconfigCluster
    #
    #     # Step 3: Read existing Kubeconfig
    #     kubeconfig = self._read(aws_client=aws_client)
    #
    #     # Step 4: Parse through the existing config to determine if
    #     # an update is required. By the end of this logic if write_kubeconfig = False
    #     # then no changes to kubeconfig are needed
    #     write_kubeconfig = False
    #
    #     if kubeconfig is None or not isinstance(kubeconfig, Kubeconfig):
    #         logger.info(f"Creating new Kubeconfig")
    #         kubeconfig = Kubeconfig(
